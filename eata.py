...
class EATA(nn.Module):
    """EATA is the implementation of EATA from https://github.com/mr-eggplant/EATA
...
def collect_params(model):
    ...
    for nm, m in model.named_modules():
        if isinstance(m, (nn.BatchNorm2d, nn.GroupNorm, nn.LayerNorm, MedBN)):
...
def configure_model(model):
    ...
    for m in model.modules():
        ...
        elif isinstance(m, MedBN):
            m.requires_grad_(True) # 只需启用梯度
