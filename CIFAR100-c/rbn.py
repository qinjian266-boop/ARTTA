...
class RBN(nn.Module):
    ...
    def find_median(self, input_data):
        """
        Calculates the median using sort to ensure determinism on GPU.
        Replaces input_reshaped.median(1)[0] which is non-deterministic.
        
        This implementation is mathematically identical to torch.median(),
        selecting the lower median for even-sized inputs.
        """
        shape = input_data.shape
        input_reshaped = input_data.transpose(1, 0).reshape(shape[1], -1)
        sorted_values, _ = torch.sort(input_reshaped, dim=1)
        mid_idx = (sorted_values.shape[1] - 1) // 2
